...
table <- df %>% 
  ...
  as_gt() %>%
  gt::gtsave("output/all_cohorts/all_cohorts_summary_table.docx")

print(table)

# Just do gidamps vs music cohort
...
table2 <- df %>% 
  ...
  add_p() %>%
  as_gt() %>%
  gt::gtsave("output/all_cohorts/cohort_1_vs_2_p_values.docx")

print(table2)

print("Complete.")

str(df$participant_location)
